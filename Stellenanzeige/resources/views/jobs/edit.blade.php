@extends('layouts.app')
@section('title','Stelle bearbeiten')
@section('content')
<div class="panel">
    <h1 style="margin-top:0; color: #10b981;">Stelle bearbeiten: {{ $job->title }}</h1>
    ...
    <form action="{{ route('jobs.update', $job) }}" method="POST">
            ...
            <div class="row">
                <label class="form-label">
                    ...
                    <select name="type" class="form-input" required>
                        <option value="">– Bitte wählen –</option>
                        <option value="Vollzeit" @selected(old('type', $job->type)=='Vollzeit')>Vollzeit</option>
                        <option value="Teilzeit" @selected(old('type', $job->type)=='Teilzeit')>Teilzeit</option>
                        <option value="Befristet" @selected(old('type', $job->type)=='Befristet')>Befristet</option>
                        <option value="Praktikum" @selected(old('type', $job->type)=='Praktikum')>Praktikum</option>
                        <option value="Ausbildung" @selected(old('type', $job->type)=='Ausbildung')>Ausbildung</option>
                        <option value="Minijob" @selected(old('type', $job->type)=='Minijob')>Minijob</option>
                        <option value="Remote" @selected(old('type', $job->type)=='Remote')>Remote</option>
                        <option value="Hybrid" @selected(old('type', $job->type)=='Hybrid')>Hybrid</option>
                    </select>
                </label>
                <label class="form-label">
        ...
        <div class="form-section">
            ...
            <label class="form-label">
                ...
                <textarea name="description" rows="6" required class="form-input"
                          placeholder="Detaillierte Beschreibung der Stelle...">{{ old('description', $job->description) }}</textarea>
            </label>
        </div>
            ...
            <label class="form-label">
                ...
                <select multiple name="category_ids[]" size="6" class="form-input">
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" @selected(collect(old('category_ids', $job->categories->pluck('id')))->contains($cat->id))>{{ $cat->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-actions">
            ...
            <a class="button secondary" href="{{ route('jobs.show', $job) }}">
